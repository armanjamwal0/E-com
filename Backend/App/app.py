...
def create_app():
    ...
    def home():
        ...
        if userId:
            ...
            user = res.scalar()
            return jsonify(
                {**(user_schema(user) if user else {}), "authenticated": True}
            )  #
    ...
    def check_auth():
        ...
        if user_id:
            ...
            print("me")
            return {
                **(user_schema(user) if user else {}),
                "authenticated": True,
            }, 200  # if user exist then return
    ...
    def register():
        ...
        user = User(name=name, email=email, password=hashed, role=UserEnum.USER)
        db.session.add(user)
        db.session.commit()
    ...
    def login():
        ...
        if not user or not check_password_hash(user.password, data["password"]):
            return {
                "authenticated": False,
                "msg": "Please Check Your email if you regisetred",
            }
        ...
        return {
            **(user_schema(user) if user else {}),
            "authenticated": True,
        }, 200  # this conver user pass and email into dic this use for testing purpose to send email to user
